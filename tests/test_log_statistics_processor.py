...
def logs():
    logs = [
        LogObject( '127.0.0.1 - user [10/Oct/2000:13:55:36 -0700] "GET /apache_pb.gif HTTP/1.0" 200 2326 "-" "Mozilla/5.0"'),
        LogObject( '127.0.0.1 - - [17/May/2015:08:05:23 +0000] "GET /downloads/product_1 HTTP/1.1" 304 230 "-" "Debian APT-HTTP/1.3 (0.8.16~exp12ubuntu10.21)"'),
        LogObject( '80.91.33.133 - - [17/May/2015:08:05:24 +0000] "GET /downloads/product_1 HTTP/1.1" 304 0 "-" "Debian APT-HTTP/1.3 (0.8.16~exp12ubuntu10.17)"'),
    ]
    return logs
...
def test_calculate_statistics_with_single_entry( log_processor):
    log = LogObject( '127.0.0.1 - user [10/Oct/2000:13:55:36 -0700] "GET /apache_pb.gif HTTP/1.0" 200 2326 "-" "Mozilla/5.0"')
...
def test_calculate_percentile( log_processor,logs):
    ...
    assert percentile == 230  # 95% перцентиль, т.к. отсортировано [0, 230, 2326] - это будет 200

...
class LogStatisticsProcessor:
    ...
    def __init__(self) -> None:
        """Инициализирует объект LogStatisticsProcessor. Создает атрибуты"""
        self.logs_count: int = 0  # Общее количество обработанных логов.
        self.resource_frequency: Counter[str] = (
            Counter()
        )  # Частота запросов к ресурсам.
        self.status_code_frequency: Counter[int] = (
            Counter()
        )  # Частота встречаемости HTTP-кодов статуса.
        self.weights_all_logs: list[int] = (
            []
        )  # Список размеров всех ответов (в байтах).
    ...
    def _calculate_percentile(self) -> int:
        ...
        """
        if len(self.weights_all_logs) == 0:
            return -1  # Обозначение, что перцентиля не существует
        self.weights_all_logs.sort()
        percentile_index = int(percentile_ratio * (len(self.weights_all_logs) - 1))
    ...
    def get_statistics(self) -> LogStatistics:
        ...
        return LogStatistics(
            ...
            average_response_size=(
                round(sum(self.weights_all_logs) / self.logs_count, 2)
                if self.logs_count
                else -1
            ),
        )

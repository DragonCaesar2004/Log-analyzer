...
class EmptyFilterValueError(ValueError):
    ...
    def __init__(self):
        super().__init__("Ошибка: Вы указали пустое поле для фильтрации, если подразумевается, что значение поля отсутствует, введите прочерк: '-'")
...
class FromAfterToError(ValueError): # Переименовать 
